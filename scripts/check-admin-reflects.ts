...
const one = async (collection: string, where: string) =>
  (await api('GET', `/${collection}?limit=1&depth=0&${where}`)).docs[0]
...
async function check(
  ...
  try {
    ...
    results.push({
      after: `ERROR ${(error as Error).message.slice(0, 120)}`,
      area,
      page: pages.join(' '),
      revert: '',
    })
...
async function run() {
  auth = (
    await api('POST', '/users/login', { email: 'dev@plumpose.local', password: 'devpassword' })
  ).token
  ...
  await check(
    ...
    ['/', '/faq', '/shop', '/products/al-shaheen-nights'],
    () =>
      setSettings({ announcementEnabled: true, announcementText: `Announcement ${MARK}` }).then(
        () => undefined,
      ),
    () =>
      setSettings({
  ...
  await check(
    'Settings → embroidery fee',
    ['/products/al-shaheen-nights', '/', '/our-story'],
  ...
  await check(
    'Settings → free delivery threshold',
    ['/shipping-returns'],
    () =>
      setSettings({ freeShippingEnabled: true, freeShippingThresholdQar: 4713 }).then(
        () => undefined,
      ),
    () =>
      setSettings({
        freeShippingEnabled: settings.freeShippingEnabled,
        freeShippingThresholdQar: settings.freeShippingThresholdQar,
      }).then(() => undefined),
    has('4,713'),
  )
  await check(
    'Settings → embroidery lead time',
    ['/'],
    () => setSettings({ personalisationLeadTime: '17–19 working days' }).then(() => undefined),
    () =>
      setSettings({ personalisationLeadTime: settings.personalisationLeadTime }).then(
        () => undefined,
      ),
    has('17–19'),
  )
  await check(
    'Settings → wheel heading',
    ['/api/spin'],
  ...
  await check(
    'Product → name (her piece)',
    ['/', '/shop', '/products/al-shaheen-nights'],
    () =>
      api('PATCH', `/products/${hers.id}`, { title: `Al Shaheen ${MARK} — Silk Pyjama Set` }).then(
        () => undefined,
      ),
  ...
  const variants = (
    await api('GET', `/variants?limit=10&depth=0&where[product][equals]=${hers.id}`)
  ).docs
  await check(
    'Sizes → price (her piece)',
    ['/', '/shop', '/products/al-shaheen-nights'],
    async () => {
      for (const v of variants) await api('PATCH', `/variants/${v.id}`, { priceInQAR: 145600 })
    },
    async () => {
      for (const v of variants)
        await api('PATCH', `/variants/${v.id}`, { priceInQAR: v.priceInQAR })
    },
    has('1,456'),
  )
  ...
  if (demo) {
    const demoVariants = (
      await api('GET', `/variants?limit=10&depth=0&where[product][equals]=${demo.id}`)
    ).docs
    // Its own page only: the shop already says "Sold out" for other pieces.
    await check(
      'Sizes → stock to 0 (demo piece)',
      [`/products/${demo.slug}`],
      async () => {
        for (const v of demoVariants) await api('PATCH', `/variants/${v.id}`, { inventory: 0 })
      },
      async () => {
        for (const v of demoVariants)
          await api('PATCH', `/variants/${v.id}`, { inventory: v.inventory })
      },
      has('Sold out'),
    )
    await check(
      'Product → unpublish (demo piece)',
      ['/shop'],
  ...
  await check(
    ...
    ['/faq'],
    () =>
      api('PATCH', `/faqs/${faq.id}`, { question: `${faq.question} ${MARK}` }).then(
        () => undefined,
      ),
  ...
  await check(
    ...
    async () => {
      pressId = (
        await api('POST', '/press', {
          date: '2026-09-01',
          headline: `Headline ${MARK}`,
    ...
    async () => {
      if (pressId) await api('DELETE', `/press/${pressId}`)
    },
    has(MARK),
  )
  ...
  await check(
    'Spotted → approve a post',
    ['/spotted', '/'],
    async () => {
      spottedId = (
        await api('POST', '/spotted', {
          caption: `Caption ${MARK}`,
          image: aPhoto.id,
          instagramHandle: `${MARK.toLowerCase()}`,
          status: 'approved',
        })
      ).doc.id
    },
    async () => {
      if (spottedId) await api('DELETE', `/spotted/${spottedId}`)
    },
  ...
  await check(
    'Reviews → approve a review',
    ['/'],
    async () => {
      reviewId = (
        await api('POST', '/reviews', {
          body: `Lovely ${MARK}`,
          email: 'e2eonly-review@plumpose.local',
          name: 'Test Reviewer',
          product: hers.id,
          rating: 5,
          status: 'approved',
    ...
    async () => {
      if (reviewId) await api('DELETE', `/reviews/${reviewId}`)
    },
    has(MARK),
  )
  ...
  if (project) {
    await check(
      'Made for You → project title',
      ['/made-for-you', `/made-for-you/${project.slug}`],
      () =>
        api('PATCH', `/projects/${project.id}`, { title: `${project.title} ${MARK}` }).then(
          () => undefined,
        ),
  ...
  const homePhotoName = (await visit('/')).html.match(
    /api\/media\/file\/([\w.-]+\.(?:jpg|png|webp))/,
  )?.[1]
  const homePhoto = homePhotoName
    ? await one('media', `where[filename][equals]=${homePhotoName}`)
    : null
  if (homePhoto) {
    await check(
      'Media → photo description (alt)',
      ['/'],
      () =>
        api('PATCH', `/media/${homePhoto.id}`, { alt: `${homePhoto.alt} ${MARK}` }).then(
          () => undefined,
        ),
              ...
              columns: [
                {
                  richText: {
                    root: {
                      children: [
                        {
                          children: [{ text: `Body ${MARK}`, type: 'text', version: 1 }],
                          type: 'paragraph',
                          version: 1,
                        },
                      ],
  ...
  await check(
    'Delivery → international zone fee',
    ['/shipping-returns'],
  ...
  const threads = (
    await api(
      'GET',
      '/personalisationOptions?limit=100&depth=0&where[type][equals]=thread&where[active][not_equals]=false',
    )
  ).docs
  const count = (p: { text: string }) =>
    Number(p.text.match(/one of (\d+) thread colours/)?.[1] ?? NaN)
  await check(
    'Embroidery → switch a thread off',
    ['/'],
    () =>
      api('PATCH', `/personalisationOptions/${threads[0].id}`, { active: false }).then(
        () => undefined,
      ),
    () =>
      api('PATCH', `/personalisationOptions/${threads[0].id}`, { active: true }).then(
        () => undefined,
      ),
    (p) => count(p) === threads.length - 1,
  )
  await check(
    'Embroidery → rename a thread',
    ['/products/al-shaheen-nights'],
    () =>
      api('PATCH', `/personalisationOptions/${threads[0].id}`, { name: `Thread ${MARK}` }).then(
        () => undefined,
      ),
    () =>
      api('PATCH', `/personalisationOptions/${threads[0].id}`, { name: threads[0].name }).then(
        () => undefined,
      ),
    hasHtml(MARK),
  )
  ...
  await check(
    'Currencies → hand-set price',
    ['/api/locale/options'],
    () => api('PATCH', `/currencies/${aed.id}`, { priceOverride: 1733 }).then(() => undefined),
    () =>
      api('PATCH', `/currencies/${aed.id}`, { priceOverride: aed.priceOverride }).then(
        () => undefined,
      ),
    hasHtml('"price":1733'),
  )
  ...
  await check(
    'Countries → blocked reason',
    ['/api/locale/options'],
    () =>
      api('PATCH', `/countries/${iran.id}`, { blockedReason: `Reason ${MARK}` }).then(
        () => undefined,
      ),
    () =>
      api('PATCH', `/countries/${iran.id}`, { blockedReason: iran.blockedReason }).then(
        () => undefined,
      ),
    hasHtml(MARK),
  )
  ...
  for (const r of results)
    console.log(`${pad(r.area, 38)} ${pad(r.page, 30)} ${pad(r.after, 18)} ${r.revert}`)
  const bad = results.filter(
    (r) =>
      !r.after.startsWith('yes, at once') || (r.revert && !r.revert.startsWith('yes, at once')),
  )
