...
const asPublic = (data: Record<string, unknown>) =>
  payload
    .create({
      collection: 'reviews',
      data: { product: productId, ...data } as never,
...
const errorsOf = async (promise: Promise<unknown>): Promise<Record<string, string>> => {
  try {
    ...
    const list =
      (error as { data?: { errors?: Array<{ message: string; path: string }> } }).data?.errors ?? []
...
afterAll(async () => {
  for (const id of reviews)
    await payload.delete({ collection: 'reviews', id, overrideAccess: true }).catch(() => undefined)
  ...
  await payload
    .delete({
      ...
      overrideAccess: true,
      trash: true,
      where: { email: { like: 'e2eonly-' } },
    })
    .catch(() => undefined)
  for (const id of orders)
    await payload.db
      .deleteOne({ collection: 'orders', where: { id: { equals: id } } })
      .catch(() => undefined)
})

describe('reviews — sent by the public', () => {
  it('always arrives pending, whatever the request claims', async () => {
    const doc = await asPublic({
      body: 'Softest thing I own. Worth every riyal.',
      email: EMAIL,
      featured: true,
      name: 'Mariam',
      rating: 5,
  ...
  it('is invisible to the public until approved', async () => {
    ...
    ).rejects.toThrow(/cannot be queried: email/)
  })

  it('checks the rating, the words and the name on the server', async () => {
    expect(
      await errorsOf(
        asPublic({ body: 'Lovely.', email: 'e2eonly-a@plumpose.local', name: '', rating: 7 }),
      ),
    ).toEqual({
  ...
  it('takes one review per person per piece', async () => {
    expect(
      await errorsOf(
        asPublic({
          body: 'Second go at the same piece.',
    ...
    expect(
      await errorsOf(
        payload.create({
          collection: 'reviews',
          data: {
            body: 'Nice nice nice nice.',
            email: 'e2eonly-b@plumpose.local',
            name: 'B',
            product: 999999,
            rating: 5,
          } as never,
  ...
  it('marks a verified purchase when that email has an order for the piece', async () => {
    const order = await payload.create({
      ...
      data: {
        amount: 139900,
        currency: 'QAR',
        customerEmail: 'E2EONLY-Buyer@plumpose.local',
    ...
    const doc = await asPublic({
      body: 'Bought it for my wedding morning.',
      email: 'e2eonly-buyer@plumpose.local',
      name: 'Noor',
      rating: 5,
    })
    expect(
      (await payload.findByID({ collection: 'reviews', id: doc.id, overrideAccess: true }))
        .verifiedPurchase,
    ).toBe(true)
  })
  ...
  it('shows once she approves it, with her reply', async () => {
    ...
    await payload.update({
      collection: 'reviews',
      data: { reply: 'Thank you, Mariam.', status: 'approved' },
      id: first,
      overrideAccess: true,
    })
    const seen = await payload.find({
