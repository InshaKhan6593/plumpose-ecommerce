...
describe('page text — the merge', () => {
  ...
  it('takes each kind of field in her words', () => {
    const merged = mergePageText({
      home: { hero: { cta: '  Shop the silk ', tagline: 'Sleep\n  beautifully. \n' }, philosophy: { headline: 'Made for the *quiet hours* of the night.' } },
      madeForYou: { offers: { bridal: { title: 'Weddings' } }, process: { steps: [{ body: 'Write to us.', title: 'One' }, { body: '', title: '' }] } },
    ...
    expect(merged.HOME.philosophy.headline).toEqual([{ text: 'Made for the ' }, { italic: true, text: 'quiet hours' }, { text: ' of the night.' }])
    expect(merged.OUR_STORY.founder.body).toEqual(['First paragraph still first.', 'Second paragraph.'])
    ...
    expect(merged.MADE_FOR_YOU.offers.map((o) => o.key)).toEqual(MADE_FOR_YOU.offers.map((o) => o.key))
  ...
  it('an emptied field goes back to the default, never blank', () => {
    const merged = mergePageText({ home: { hero: { cta: '   ', tagline: '\n\n' } }, ourStory: { silk: { pillars: [] } } })
  ...
  it('writes and reads italics the same way', () => {
    ...
    expect(text).toBe('Inspired by the *tranquil waters* of Qatar and the *seasonal gathering* of whale sharks.')
...
describe('page text — saved in the admin', () => {
  ...
  beforeAll(async () => {
    ...
    before = (await payload.findGlobal({ depth: 0, slug: 'pageText' })) as unknown as Record<string, unknown>
  }, 120_000)

  afterAll(async () => {
    const { createdAt: _c, globalType: _g, id: _i, updatedAt: _u, ...data } = before as Record<string, unknown>
    await payload.updateGlobal({ context: { disableRevalidate: true }, data: data as never, slug: 'pageText' })
  })
  ...
  it('what she saves is what the page gets', async () => {
    await payload.updateGlobal({
      ...
      data: { home: { hero: { cta: 'See the set', tagline: 'Rest\nbeautifully.' } }, press: { heading: 'In the press' } } as never,
      slug: 'pageText',
    })
    const merged = mergePageText((await payload.findGlobal({ depth: 0, slug: 'pageText' })) as unknown as Record<string, unknown>)
