...
describe('rates — fetching', () => {
  it('reads QAR rates and the time they are from', async () => {
    const rates = await fetchRates(
      reply({
        base_code: 'QAR',
        rates: { AED: 1.008929, GBP: 0.207814, QAR: 1, bad: -1, XXXX: 2 },
        result: 'success',
        time_last_update_unix: 1790208151,
      }),
    )
  ...
  it('refuses rates on another base, or an error, rather than writing wrong numbers', async () => {
    await expect(
      fetchRates(reply({ base_code: 'USD', rates: { AED: 3.67 }, result: 'success' })),
    ).rejects.toThrow(/QAR/)
...
describe('rates — checking her hand-set price', () => {
  it('shows what today’s rate would make the anchor piece, and how far her price is from it', () => {
    ...
    expect(
      formatDifference(rateCheck({ anchorQar: 1399, price: 330, rate: 0.241396 })!.difference),
    ).toBe('−2.3%')
    expect(
      formatDifference(rateCheck({ anchorQar: 1399, price: 300, rate: 0.207814 })!.difference),
    ).toBe('+3.2%')
  })
