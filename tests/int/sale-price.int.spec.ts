...
beforeAll(async () => {
  ...
  const piece = (
    await payload.find({
      collection: 'products',
      limit: 1,
      where: { slug: { equals: 'demo-noir-slip' } },
    })
  ).docs[0]
...
afterAll(async () => {
  await payload.update({
    ...
    data: { compareAtPriceInQAR: before ?? null },
    id,
    overrideAccess: true,
  })
})
...
describe('sale price', () => {
  it('takes a was-price higher than the price', async () => {
    const saved = await payload.update({
      collection: 'products',
      context: { disableRevalidate: true },
      data: { compareAtPriceInQAR: 95000 },
      id,
      overrideAccess: true,
    })
  ...
  it('refuses one at or below the price — that would not be a sale', async () => {
    const error = await payload
      .update({
        collection: 'products',
        context: { disableRevalidate: true },
        data: { compareAtPriceInQAR: 75000 },
        id,
        overrideAccess: true,
      })
      .then(() => null)
