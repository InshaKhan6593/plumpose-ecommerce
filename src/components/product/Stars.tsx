 ...
 */
export function Stars({ className, size = 14, value }: { className?: string; size?: number; value: number }) {
  const pct = Math.max(0, Math.min(100, (value / 5) * 100))
  const row = (filled: boolean) => (
    <span className="flex gap-[3px]">
      {Array.from({ length: 5 }, (_, i) => (
        <svg aria-hidden fill={filled ? 'currentColor' : 'none'} height={size} key={i} stroke="currentColor" strokeWidth={1.2} viewBox="0 0 24 24" width={size}>
  ...
  return (
    <span className={cn('relative inline-flex text-ink', className)} role="img" aria-label={`${Number(value.toFixed(1))} out of 5 stars`}>
      {row(false)}
