...
export function ProductReviews({ productId, reviews }: { productId: number; reviews: Review[] }) {
  ...
  return (
    <section
      ...
      className="mx-auto max-w-[90rem] scroll-mt-28 px-4 pt-24 md:px-7 md:pt-32"
      id="reviews"
    >
      <Reveal className="grid gap-12 border-t border-line pt-14 lg:grid-cols-[minmax(0,20rem)_minmax(0,1fr)] lg:gap-20">
        <div data-reveal>
          <h2
            className="serif-display text-[clamp(2.25rem,3.6vw,3.25rem)] leading-[1.05]"
            id="reviews-heading"
          >
            In your words
          </h2>
          {average !== null ? (
            ...
            <p className="mt-6 text-[0.9375rem] leading-relaxed text-ink-soft">
              No reviews yet. If you have worn this piece, we would love to hear.
            </p>
          )}
          <div className="mt-10">
              ...
              <li className="py-9 first:pt-0" data-reveal key={review.id}>
                <div className="flex flex-wrap items-center gap-x-5 gap-y-2">
                  ...
                  {review.verifiedPurchase ? (
                    <span className="caps text-[0.5625rem] text-ink-soft">Verified purchase</span>
                  ) : null}
                </div>
                {review.title ? (
                  <h3 className="serif-display mt-4 text-[1.5rem] leading-snug">{review.title}</h3>
                ) : null}
                <p className="mt-3 max-w-2xl text-[0.9375rem] leading-[1.75] whitespace-pre-line text-ink">
                  {review.body}
                </p>
                ...
                {review.reply ? (
                  <div className="mt-6 max-w-2xl border-l border-ink pl-5">
                    ...
                    <p className="mt-2 text-[0.9375rem] leading-[1.75] whitespace-pre-line text-ink-soft">
                      {review.reply}
                    </p>
                  </div>
                ) : null}
