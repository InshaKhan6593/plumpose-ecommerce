...
export function MadeSteps({
  copy = HOME.steps,
  steps,
}: {
  copy?: typeof HOME.steps
  steps: Step[]
}) {
  ...
  useLayoutEffect(() => {
    ...
    const stop = whenReached(el, 2, () =>
      ctx.add(() => {
         ...
         */
        el.querySelectorAll<HTMLImageElement>('[data-step-frame] img').forEach((img) =>
        ...
        for (let i = 1; i < n; i++) {
          ...
          tl.fromTo(
            ...
            { duration: CHANGE * 1.3, ease: 'power2.out', scale: 1.1, xPercent: 0 },
            at,
          )
          tl.to(
            pictures[i - 1],
            { duration: CHANGE, ease: 'power2.inOut', scale: 1.04, xPercent: 8 },
            at,
          )
          ...
          tl.fromTo(
            titles[i],
            { opacity: 0, x: -28 },
            { duration: CHANGE * 0.5, ease: 'sine.out', opacity: 1, x: 0 },
            i,
          )
          tl.to(
            bodies[i - 1],
            { duration: CHANGE * 0.4, ease: 'sine.in', opacity: 0, x: 20 },
            at + 0.03,
          )
          tl.fromTo(
            bodies[i],
  ...
  const count = (n: number) =>
    `${String(n + 1).padStart(2, '0')} / ${String(steps.length).padStart(2, '0')}`
            ...
            <div className="mx-auto grid w-full max-w-[90rem] grid-cols-1 items-center gap-5 px-6 lg:grid-cols-[1fr_minmax(0,26rem)_1fr] lg:gap-12 lg:px-7">
              ...
              <div className="relative order-2 h-20 overflow-hidden lg:order-1 lg:h-36">
                {steps.map((step, i) => (
                  <div className="absolute inset-0" data-step-title key={step.title}>
                    <h3 className="serif-italic text-[clamp(1.75rem,2.6vw,2.5rem)] leading-tight">
                      {step.title}
                    </h3>
                    <p className="caps mt-4 text-[0.5625rem] text-ink-soft tabular-nums">
                      {count(i)}
                    </p>
                  </div>
                ))}
              ...
              */}
              ...
              <div className="relative order-3 h-24 overflow-hidden lg:h-32">
                {steps.map((step) => (
                  <p
                    className="absolute inset-0 max-w-xs text-[0.9375rem] leading-relaxed text-ink-soft lg:ml-auto"
                    data-step-body
                    key={step.title}
                  >
                    {step.body}
