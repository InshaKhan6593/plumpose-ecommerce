...
export function AddressBook({
  ...
  const countryName = (code?: null | string) =>
        ...
        <ul className="grid gap-6 md:grid-cols-2">
          {addresses.map((address) =>
            editing === address.id ? (
              ...
              <li
                className="flex flex-col justify-between gap-6 border border-line px-6 py-6"
                key={address.id}
              >
                <address className="text-[0.9375rem] leading-[1.7] not-italic">
                  ...
                  {address.addressLine2 ? (
                    <span className="block text-ink-soft">{address.addressLine2}</span>
                  ) : null}
                  ...
                  {address.phone ? (
                    <span className="mt-2 block text-ink-soft tabular-nums">{address.phone}</span>
                  ) : null}
                </address>
...
function AddressForm({
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    const body = Object.fromEntries(
      Object.entries({ ...data, postalCode: inQatar ? '' : data.postalCode }).map(([k, v]) => [
        k,
        v.trim(),
      ]),
    )
  ...
  return (
    <form
      className="grid gap-8 border border-line px-6 py-8 sm:grid-cols-2"
      noValidate
      onSubmit={onSubmit}
    >
      <h3 className="caps text-[0.625rem] sm:col-span-2">
      ...
      <HouseField error={errors.firstName?.message} id="a-first" label="First name">
        <input
          ...
          id="a-first"
          {...register('firstName', required('a first name'))}
        />
      </HouseField>
      <HouseField error={errors.lastName?.message} id="a-last" label="Last name">
        <input
          ...
          className={houseInput}
          id="a-last"
          {...register('lastName', required('a last name'))}
        />
      </HouseField>
      ...
      >
        <input
          ...
          className={houseInput}
          id="a-line1"
          {...register('addressLine1', required('a street and building'))}
        />
      </HouseField>
      <HouseField
        className="sm:col-span-2"
      ...
      <div className="flex flex-wrap items-center gap-6 sm:col-span-2">
        <HouseButton disabled={isSubmitting}>
