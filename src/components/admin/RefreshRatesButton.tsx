...
export const RefreshRatesButton: React.FC = () => {
  ...
  const [state, setState] = useState<{ busy: boolean; message: string }>({
    busy: false,
    message: '',
  })
  ...
  const refresh = async () => {
    ...
    try {
      const res = await fetch('/api/currencies/refresh-rates', {
        credentials: 'include',
        method: 'POST',
      })
      ...
      const when = body.ratesFrom
        ? new Date(body.ratesFrom).toLocaleString('en-GB', {
            dateStyle: 'medium',
            timeStyle: 'short',
            timeZone: 'Asia/Qatar',
          })
      ...
      setState({
        busy: false,
        message: `${body.updated} rates updated${when ? `, as of ${when} (Doha)` : ''}.`,
      })
      router.refresh()
    } catch {
      setState({
        busy: false,
        message: 'The rate service could not be reached. Please try again later.',
      })
    }
  }
