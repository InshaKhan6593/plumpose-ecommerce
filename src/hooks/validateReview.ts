...
export const validateReview: CollectionBeforeValidateHook = async ({ data, operation, req }) => {
  ...
  const productId = typeof data.product === 'object' && data.product ? (data.product as { id: unknown }).id : data.product
  ...
  if (!Number.isInteger(rating) || rating < 1 || rating > 5) errors.push({ message: 'Please choose from one to five stars.', path: 'rating' })
  if (body.length < 10) errors.push({ message: 'Please write a few words — at least a sentence.', path: 'body' })

  const product = productId
    ? await req.payload
        .find({ collection: 'products', depth: 0, limit: 1, req, where: { and: [{ id: { equals: productId } }, { _status: { equals: 'published' } }] } })
        .then((r) => r.docs[0])
        .catch(() => undefined)
    : undefined
  ...
  if (!errors.length && product) {
    ...
    if (already.totalDocs) errors.push({ message: 'You have already reviewed this piece — thank you.', path: 'email' })
  }
  ...
  if (!errors.length && device) {
    ...
    if (recent.totalDocs >= REVIEWS_PER_DEVICE_PER_DAY) errors.push({ message: 'Thank you — that is enough reviews from here for today.', path: 'body' })
  }
...
async function boughtIt(req: PayloadRequest, email: string, productId: number): Promise<boolean> {
  const account = await req.payload
    .find({ collection: 'users', depth: 0, limit: 1, overrideAccess: true, req, where: { email: { equals: email } } })
    .then((r) => r.docs[0])
    .catch(() => undefined)
  if (account) {
    const viaAccount = await req.payload.count({
      ...
      overrideAccess: true,
      req,
      where: { and: [{ 'items.product': { equals: productId } }, { customer: { equals: account.id } }] },
    })
  ...
  const asGuest = await req.payload.find({
    ...
    where: { and: [{ 'items.product': { equals: productId } }, { customerEmail: { like: email } }] },
  })
