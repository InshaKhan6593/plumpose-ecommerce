...
const valueOf = (rows: Row[] | null | undefined, name: string) =>
  (rows ?? []).find((r) => r.field === name)?.value?.trim() ?? ''

export function enquirySummary(rows: Row[] | null | undefined): {
  about: string
  from: string
  preview: string
} {
  ...
  return {
    about:
      [subject, order ? `order ${order.startsWith('#') ? order : `#${order}`}` : '']
        .filter(Boolean)
        .join(' · ') || '—',
