...
export default async function OrderPage({
  ...
  const stepIndex = Math.max(
    0,
    STEPS.findIndex((s) => s.key === (order.fulfilment ?? 'unfulfilled')),
  )
    ...
    <div className="mx-auto max-w-3xl px-5 pt-14 pb-28 md:pt-20">
      ...
      <header className="text-center">
        {justPlaced ? (
          <span
            aria-hidden
            className="mx-auto flex size-12 items-center justify-center rounded-full border border-ink"
          >
        ...
        <h1
          className={cn(
            'serif-display text-[clamp(2.5rem,5vw,3.75rem)] leading-[1.05]',
            justPlaced && 'mt-8',
          )}
        >
          {justPlaced
            ? `Thank you${address?.firstName ? `, ${address.firstName}` : ''}.`
            : 'Your order'}
        </h1>
        {justPlaced ? (
      ...
      <section aria-label="Your pieces" className="mt-16 bg-paper-3 px-6 py-8 md:px-10">
        ...
        <dl className="mt-6 flex flex-col gap-2.5 text-sm">
          {typeof order.subtotalQar === 'number' ? (
            <Row label="Pieces" value={formatQar(order.subtotalQar)} />
          ) : null}
          {hasEmbroidery ? (
            <Row label="Embroidery" value={formatQar(order.personalisationTotalQar)} />
          ) : null}
        ...
        {order.gift ? (
          <div className="sm:col-span-2">
            ...
            <p className="serif-italic mt-3 text-lg leading-relaxed">
              {order.giftNote
                ? `“${order.giftNote}”`
                : 'With a handwritten card, and no prices in the parcel.'}
            </p>
          </div>
        ) : null}
      </section>
      ...
      <footer className="mt-16 flex flex-col items-center gap-5 border-t border-line pt-10 text-center">
        <Link
          className="caps bg-ink px-10 py-4 text-[0.6875rem] text-white hover:bg-ink/85"
          href="/shop"
        >
          Continue shopping
        </Link>
