...
export default async function PressPage() {
  ...
  const pressHref = settings.contactEmail
    ? `mailto:${settings.contactEmail}?subject=Press%20enquiry`
          ...
          <ul className="mx-auto mt-16 max-w-5xl border-t border-line md:mt-24">
            {items.map((item) => {
              ...
              return (
                <Reveal
                  as="li"
                  className="grid gap-6 border-b border-line py-10 md:grid-cols-[12rem_minmax(0,1fr)] md:gap-14 md:py-14"
                  key={item.id}
                >
                  ...
                  <div>
                    <h2
                      className="serif-display text-[clamp(1.75rem,2.6vw,2.5rem)] leading-tight"
                      data-reveal
                    >
                      {item.headline}
                    </h2>
                    {item.excerpt ? (
                      <p
                        className="mt-4 max-w-2xl text-[0.9375rem] leading-[1.8] text-ink-soft"
                        data-reveal
                      >
                        {item.excerpt}
                      </p>
