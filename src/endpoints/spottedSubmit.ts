...
export async function checkSubmission(input: {
  ...
  errors: Errors
  value?: { caption: string; email: string; format: string; handle: string; postUrl: string }
}> {
  const errors: Errors = {}
  const d = input.data
  ...
  if (!handle || !HANDLE.test(handle))
    errors.handle = 'Please add your Instagram name, like @yourname.'
  if (postUrl && !INSTAGRAM.test(postUrl))
    errors.postUrl = 'That should be a link to the post on Instagram.'
  ...
  if (!file?.data?.length) errors.file = 'Please choose a photograph.'
  else if ((file.size ?? file.data.length) > MAX_BYTES)
    errors.file = 'That photograph is over 12 MB — please send a smaller one.'
  else {
    ...
    else if (Math.min(meta.width ?? 0, meta.height ?? 0) < MIN_SIDE)
      errors.file = 'That photograph is too small to show well — please send a larger one.'
  }

  return Object.keys(errors).length
    ? { errors }
    : { errors, value: { caption, email, format, handle, postUrl } }
}

export const spottedSubmitEndpoint: Endpoint = {
  handler: async (req: PayloadRequest) => {
    await addDataAndFileToRequest(req)
    const { errors, value } = await checkSubmission({
      data: (req.data ?? {}) as Record<string, unknown>,
      file: req.file as never,
    })
    ...
    if (device) {
      ...
      if (recent.totalDocs >= SPOTTED_PER_DEVICE_PER_DAY)
        return json(
          { errors: { file: 'Thank you — that is enough photographs from here for today.' } },
          429,
        )
    }
    ...
    const media = await req.payload.create({
      ...
      file: {
        ...
        name: `spotted-${Date.now()}.${value.format}`,
        size: file.size,
      },
      overrideAccess: true,
      req,
    ...
    try {
      ...
      await req.payload
        .delete({ collection: 'media', id: media.id, overrideAccess: true, req })
        .catch(() => undefined)
      throw error
    }
